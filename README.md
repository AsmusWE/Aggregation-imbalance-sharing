...
│   ├── Export_functions.jl   # Write results as Parquet/JSON for the Python figures
...
│   ├── ExportPlotData.jl     # Re-export a result cache for the Python figures
│   ├── ExportSystemData.jl   # Export the raw input data for the Python figures
│   ├── TestTheoreticalResults.jl  # Validate the paper's analytical results on the data
│   └── FigureScripts/        # Standalone thesis-figure scripts
├── python/                   # Figure generation (PNG + LaTeX .pgf for the paper)
│   ├── plotting_main.py      # Entry point; reads the exports written from Julia
│   ├── data_io.py            # Loaders and pure-computation helpers
│   ├── style.py              # Shared palette and paper-matched figure style
...
│   └── cache_py/              # Gitignored Parquet/JSON exports read by python/
└── docs/                      # Thesis report
```
...
2. From the repository root, instantiate the environment:
   ...
   Pkg.instantiate()
   ```
3. Run the main analysis. This computes coalition costs and allocations, then serializes
   the result to `results/cache/AllClientMonthlyNucleolus.jls`:
   ```
   julia --project=. scripts/Imbalance_main.jl
   ```
4. Generate figures from that cache, by either route:

   **Julia** (quick on-screen plots):
   ```
   julia --project=. scripts/Plotting_main.jl
   ```

   **Python** (the figures used in the paper — PNG plus LaTeX-typeset `.pgf`):
   ```
   pip install -r python/requirements.txt
   julia --project=. scripts/ExportPlotData.jl
   julia --project=. scripts/ExportSystemData.jl
   python python/plotting_main.py
   ```
   The Julia export steps write Parquet/JSON under `results/cache_py/`, since Julia's
   serialization format is not readable from Python.

Run the test suite with `julia --project=. test/runtests.jl`. The tests use synthetic
fixtures only and do not require the private data.
...
The default preset (`ALLOCATION_PRESETS[:default]`) is `["shapley", "MCC", "VCG", "gately", "marginal_price", "flat_rate"]`; `nucleolus` and `shapley` become computationally intractable above roughly 12 and 19 clients respectively, since both require evaluating every coalition.
