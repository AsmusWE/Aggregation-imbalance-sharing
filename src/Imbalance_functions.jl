...
function calculate_bids(coalitions, system_data, stochastic_data; full_opt = false, one_price = false, use_newsvendor = true)
    ...
    # The union over all coalitions, since callers may pass a partial batch that omits some
    # singletons, and every referenced client still needs its own bid below. Not
    # keys(client_pv_ownership): clients the caller filtered out have no demand scenarios.
...
function calculate_imbalances_specific(system_data, coalitions, stochastic_data, sim_days; dummy = false, one_price = false, use_newsvendor = true, full_opt = false)
    ...
    T = sim_days * 24
    # All clients referenced anywhere in `coalitions`. The union rather than the largest single
    # coalition, since a partial batch may omit the grand coalition. Not
    # keys(client_pv_ownership): clients the caller filtered out have no demand scenarios.
