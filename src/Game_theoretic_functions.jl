using Combinatorics, HiGHS, JuMP, LinearAlgebra, Serialization
...
function calculate_allocations(
    ...
    for allocation in allocations
        if haskey(allocation_map, allocation)
            # Time the single computation whose result is kept
...
function nucleolus(clients, coalition_costs; checkpoint_path::Union{Nothing, AbstractString} = nothing,
    ...
    # Nucleolus via sequential LP peeling with row (cut) generation. Materializing all
    # 2^n_clients - 1 coalitions as explicit LP rows is intractable beyond ~19 clients:
    # presolving a model of that size costs seconds per iteration regardless of how easy the
    # actual LP is (most rows are slack at any given solution), and degenerate instances can
    # need iteration counts approaching n_coalitions.
    #
    # Instead each stage's LP starts from only the coalitions known to matter (those locked by
    # previous stages, plus the cuts added so far) and is solved by cutting-plane / row
    # generation:
    #  - solve the current (small) master LP for (x*, max_excess*)
    #  - compute the excess of EVERY coalition at x* directly (no solver involved: a single
    #    O(n_coalitions) pass using a lowest-set-bit recursion to get sum(x*) over every
    #    subset, then a vectorized subtract against the precomputed coalition costs)
    #  - if some coalition's true excess exceeds max_excess*, the master is missing a binding
    #    constraint: add ALL such violated coalitions as new <= rows in one batch and resolve.
    #    Batching rather than adding only the worst violator avoids the slow "zigzagging"
    #    convergence of one-cut-at-a-time Kelley cutting planes; since the master has only
    #    n_clients + 1 variables, a handful of rounds pins down each stage's optimum.
    ...
    #    x* is feasible for the true problem at that same bound
    #  - tied coalitions are then added to the model if not already present, the LP is resolved
    #    once so duals are valid, and any coalition with a nonzero dual (tight in every optimal
    #    solution, not just the returned vertex) is locked by swapping its row for an equality
    #    at the achieved value
    #
    # A coalition, once added as a cut, is never removed, so the total number of cuts over the
    # whole run is bounded by n_coalitions regardless of how the per-stage rounds behave; in
    # practice only a small fraction of coalitions ever become active constraints.
    #
    # Optional checkpointing: pass checkpoint_path to persist the locked set (via Serialization,
    # not the JuMP model itself) every checkpoint_every stage-lock events. If that file exists
    # when called, it is loaded and the run resumes from the saved locked set. This makes a run
    # recoverable across an HPC wall-time limit or an interrupted desktop run; without it, no
    # progress is observable or reusable until the function returns.
    ...
    # forces max_excess up), and singletons are the cheapest bounding set.
    for mask in 1:n_coalitions
    ...
    while resolve_count < max_resolves
        ...
        if true_max > max_excess_val + cut_tol
            ...
            # already in the model, then resolve.
            for m in active_masks
