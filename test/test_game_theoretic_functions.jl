...
#
# Not covered here: the eq.-13 identity (VCG = MCC - MP) and the Corollary-A.2 sign check.
# Both are exercised against the real case-study data by scripts/TestTheoreticalResults.jl,
# which needs the full per-hour imbalance series that these synthetic fixtures do not model.

@testset "Game_theoretic_functions.jl" begin
    @testset "mcc_allocation" begin
        clients = ["A", "B", "C"]

        # Hand-built additive coalition_costs for a 3-client example:
        # c(S) = sum(c([i]) for i in S), with c(A) = -10, c(B) = -20, c(C) = -30
    ...
    @testset "shapley_value" begin
        ...
        # φ_i(c) = sum_{S ∋ i} (|S|-1)! (n-|S|)! / n! * (c(S) - c(S\{i})), with n = 3:
        #   S={A}:     weight = 0!·2!/3! = 1/3, contribution = c(A)   - c(∅)  = -1 - 0    = -1
        #   S={A,B}:   weight = 1!·1!/3! = 1/6, contribution = c(AB)  - c(B)  = -4 - (-2) = -2
        #   S={A,C}:   weight = 1/6,            contribution = c(AC)  - c(C)  = -5 - (-3) = -2
        #   S={A,B,C}: weight = 2!·0!/3! = 1/3, contribution = c(ABC) - c(BC) = -7 - (-6) = -1
        # φ_A = (1/3)(-1) + (1/6)(-2) + (1/6)(-2) + (1/3)(-1) = -4/3
        expected_A = -4.0 / 3.0
    ...
    @testset "check_stability" begin
        ...
        # Excess is the largest gap between a coalition's stand-alone cost and the sum
        # allocated to its members. {A,B} is binding here: -25 - (-15 + -20) = 10, i.e. A
        # and B together are allocated more than the coalition would cost on its own.
        @test max_excess isa Real
        @test isapprox(max_excess, 10.0; atol=1e-10)

        println("check_stability (max excess): PASS")
    end
    ...
    @testset "calculate_allocations" begin
        ...
        # Every name in `allocations` must resolve to a mechanism in allocation_map
        ...
        println("calculate_allocations (single allocation): PASS")
        ...
        # return_time=true returns (allocation_costs, allocation_times)
