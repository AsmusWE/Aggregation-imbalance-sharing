...
#
# Not covered here: calculate_wmape, which branches on `length(coalition) == 1` and so
# requires single-character client codes when passed a client name as a String; exercising
# it meaningfully needs the real client codes rather than these synthetic fixtures.
