...
# =========================
# Name of the serialized result cache written at the end of this script; the plotting
# entry points read the same name (see scripts/Plotting_main.jl, scripts/ExportPlotData.jl).
file_name = "AllClientMonthlyNucleolus.jls"
...
# (2^|clients| - 1 coalitions) at full-year hourly resolution does not fit.
# Batching by coalition keeps the time axis intact: system_data/stochastic_data are sliced to
# the full year exactly once, up front, and every batch reuses that same slice. calculate_bids
# computes a multi-client coalition's bid as the sum of its members' independently-computed
# singleton bids (no shared state across coalitions), so batching the coalition list changes
# only the peak memory, not the numbers.
