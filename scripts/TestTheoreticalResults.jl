...
# against the year-2024 case study data.
#
# Only the sparse coalition set is needed (singletons, grand coalition, leave-one-out
# coalitions): MCC, VCG, and marginal price all have O(|N|) complexity (Table I), and none
# of the checks below need Shapley or the Nucleolus.
#
...
# =========================
# 1. Data Loading & Setup (mirrors Imbalance_main.jl)
# =========================
system_data, clients, demand_data = load_data()
# load_data() returns the clients with complete data. Use CLIENT_EXCLUSION_PRESETS[:none] to
# keep all of them: the other presets assume a fuller snapshot and would drop consumers that
# load_data() has already excluded.
...
# =========================
# Costs are computed in a single full-year call rather than in chunked periods. The joined
# input series has a 48-hour outage (2024-04-12T21:00 to 2024-04-14T22:00 is one step), so a
# fixed-row-count window spanning it reaches past its calendar end; chunking that re-anchors
# each period on a calendar date then re-reads those rows. Slicing exactly once avoids the
# question entirely, and the sparse coalition set is small enough that memory is not a concern.
# See socializedVsIndividualizedCosts.jl for the chunked variant, which advances by row
# position so that its periods tile the frame.
grand_coalition = vec(clients)
coalitions = sparse_coalitions(clients)   # 2|N| + 1 coalitions, not the full power set
...
for t in 1:T
    ...
    # equal c_t_N exactly, since both are c_t(N) computed two different ways. A sign mismatch
    # between hour_cost's and lambda's conventions makes this nonzero and fires the WARNING
    # printed below.
    ...
    for c in clients
        ...
        # opposite signs (harms), else 0. lambda is the negative of the raw `spread` column:
        # the codebase's cost identity is `min(0, Δ*spread)` (negative-for-cost), while
        # `hour_cost` above and the rest of this loop use the paper's cost >= 0 convention.
        # Using +spread would give lambda*Δ = -hour_cost, mixing the two conventions; -spread
        # makes lambda*Δ == hour_cost(Δ, t) exactly, as the self-check above verifies.
...
# Cross-check: manual per-hour VCG reimplementation (eq. 10b, paper convention) vs
# vcg_allocation (the codebase's negative-for-cost convention, same as mcc_allocation). Report
# which direction matches rather than silently taking the min, so a sign regression shows up as
# a changed direction rather than just a small residual.
