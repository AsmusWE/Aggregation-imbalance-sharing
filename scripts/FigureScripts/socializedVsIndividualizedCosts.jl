...
# Drop the two smallest clients to keep the full power set of coalitions tractable
...
#
# Periods advance by row position through the already-sliced simulation frame rather than by
# re-anchoring each period on a calendar date. The two are not equivalent: the input data has
# a 48-hour outage (2024-04-12T21:00 to 2024-04-14T22:00 is a single step in the joined
# series), so a fixed 7*24-row window that spans the outage reaches 48 rows past its calendar
# end. Re-anchoring the next period by date then re-reads those 48 rows, double-counting them
# and leaving the final 48 rows of the frame unused. Stepping the cursor by exactly the number
# of rows consumed makes the periods tile the frame: every interval is priced once, and the
# accumulated imbalance series lines up with the price series used by calculate_allocations.
reconciliation_period_days = 7
intervals_per_period = reconciliation_period_days * 24
period_datetimes = system_data["price_prod_demand_df"][!, :HourUTC_datetime]
total_intervals = length(period_datetimes)
num_periods = Int(ceil(total_intervals / intervals_per_period))

for period in 1:num_periods
    first_row = (period - 1) * intervals_per_period + 1
    last_row = min(first_row + intervals_per_period - 1, total_intervals)
    days_in_period = (last_row - first_row + 1) ÷ 24
    period_start = period_datetimes[first_row]

    println("Processing period $period of $num_periods ($period_start to $(period_datetimes[last_row]))")

    # Create system data for this period
    period_system_data = Dict{String, Any}(
        "client_pv_ownership" => system_data["client_pv_ownership"],
        "price_prod_demand_df" => system_data["price_prod_demand_df"][first_row:last_row, :],
    )
...
# This script runs the simulation/optimization and exports the plot-ready result; the figure
# itself is drawn from Python (see python/figures/socialized_vs_individualized.py).
