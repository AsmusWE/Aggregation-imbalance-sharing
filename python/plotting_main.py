"""Entry point for generating the paper's figures.

Reads the Parquet/JSON/CSV exports written by the Julia simulation:

  julia scripts/ExportPlotData.jl [cache_file_name]   (default AllClientMonthlyNucleolus.jls)
  julia scripts/ExportSystemData.jl
  julia scripts/FigureScripts/socializedVsIndividualizedCosts.jl

Each figure is saved under results/figures/ as both a PNG and a LaTeX-typeset .pgf sized
for direct \\input{} into the paper (see style.save_figure).
