"""Shared color palette and default figure style for the generated figures.

Colors mirror the PALETTE_* constants in src/Plotting_functions.jl: they encode which
allocation mechanism is which, so the two implementations must agree. Figure sizes and
fonts are set here for the paper's layout (see PGF_RC) rather than copied from the Julia
figures."""
...
# this with system fonts. Must be applied globally in apply_theme() rather than only around
# the .pgf save: axis and tick labels bake in rcParams['font.family'] when ax.set_xlabel()
# etc. is called, not at savefig time.
PGF_RC = {
...
def linear_gradient(hex_start, hex_end, n):
    """n colors linearly interpolated between two hex colors (inclusive)."""
...
def four_color_gradient(n_clients, cost_order):
    """A light_green -> sand -> orange -> red gradient across clients, ranked by cost.
    `cost_order` is a 0-indexed list giving, for each rank position (low to high final
    cost), the client's index; returns one color per client, ordered to match the original
    `clients` list."""
    if n_clients == 1:
