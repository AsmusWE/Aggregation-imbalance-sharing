...
Also hosts Python equivalents of the pure-computation helpers from src/*.jl that the
figures need but that don't require re-running any Julia simulation: sort_clients_by_demand,
create_alphabetic_client_mapping and scale_distribution (src/Plotting_functions.jl),
check_stability (src/Game_theoretic_functions.jl), and calculate_wmape
(src/Imbalance_functions.jl).
"""
...
def coalition_key(coalition) -> str:
    """Canonical string key for a coalition (sorted, comma-joined client names), matching
    coalition_key() in src/Export_functions.jl."""
...
def load_mixed_allocation() -> tuple[pd.DataFrame, list[str], float | None]:
    """Load the export written by socializedVsIndividualizedCosts.jl.
...
def sort_clients_by_demand(price_prod_demand_df: pd.DataFrame, clients: list[str]) -> list[str]:
    """Clients ordered by total demand, highest first."""
...
def create_alphabetic_client_mapping(clients: list[str]) -> dict[str, str]:
    """Maps each client, in the given order, to a display code A, B, ..., Z, AA, AB, ..."""
...
def scale_distribution(
    ...
    """Cost per MWh of demand for each client."""
...
def check_stability(client_costs: dict[str, float], coalition_costs, clients: list[str]) -> float:
    """Max excess of any proper sub-coalition's stand-alone cost over its share of
    client_costs. `coalition_costs` may be keyed by frozenset/tuple/list of client names
    (any iterable works)."""
...
def calculate_wmape(
    ...
    """Weighted mean absolute percentage error of a coalition's imbalance against its net
    demand. `coalition` is either a single client name (str) or an iterable of client names.
    Note that the Julia calculate_wmape branches on `length(coalition) == 1`, which for a
    string is its character count; it therefore requires single-character client codes,
    whereas this accepts either form explicitly."""
