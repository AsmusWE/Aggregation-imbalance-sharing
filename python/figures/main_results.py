"""The main per-consumer result figures: allocated cost per MWh, PV coverage, cost vs
forecast error, and allocated-versus-standalone cost ratios.

Counterparts of plot_results() and plot_cost_difference() in src/Plotting_functions.jl.
The "Imbalance cost per MWh vs PV Coverage" scatter available there is not included here;
"Imbalance Cost Ratio vs PV Coverage" and the PV-coverage bar chart are distinct plots and
are included below.
"""
...
def _plot_socialized_vs_individualized(mixed_allocation_df, clients, demand_df, negative_excess_step=None):
    """Cost per MWh against individualization grade, one line per consumer."""
...
def plot_results(
    ...
    # Plot in the order given by `allocations`, restricted to those we have costs for
    allocations = [a for a in allocations if a in allocation_costs]
    ...
    if highlighted_keys:
        ...
        fig, ax = plt.subplots(figsize=(default_width, default_height/2))
